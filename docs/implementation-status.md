...
| Application packages use HTTP port interface only | Done  | [internal/port/http.go](../internal/port/http.go); `channel`, `queue` use `port.HTTPClient`; `auth` uses `HTTPDoer` |
